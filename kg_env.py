...
class BatchKGEnvironment(object):
    ...
    def _get_actions(self, path, done):
        ...
        for r, next_node_id in candidate_acts:
            ...
            elif next_node_type == ARTICLE:
                src_embed = user_embed + self.embeds[RECOMMENDED][0]
            elif next_node_type == WORD:
                src_embed = user_embed + self.embeds[WITHIN][0]
            else:  # BRAND, CATEGORY, RELATED_PRODUCT
                src_embed = user_embed + self.embeds[RECOMMENDED][0] + self.embeds[r][0]
    ...
    def _get_reward(self, path):
        ...
        target_score = 0.0
        _, curr_node_type, curr_node_id = path[-1]
        if curr_node_type == ARTICLE:
            # Give soft reward for other reached products.
            uid = path[0][-1]
            u_vec = self.embeds[USER][uid] + self.embeds[RECOMMENDED][0]
            p_vec = self.embeds[ARTICLE][curr_node_id]
